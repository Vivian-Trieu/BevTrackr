...
function togglePingStatus(hole) {
    ...
    firebase.database().ref('ElPradoChinCreekBev/hole' + hole + '/ping').once('value').then(function (snapshot) {
        var currentStatus = snapshot.val();

        // Update the ping status only if it is currently true
        if (currentStatus === true) {
            firebase.database().ref('ElPradoChinCreekBev/hole' + hole).update({
                ping: false
            });
        }
    });
...
function listenForPingChanges(hole, button) {
    firebase.database().ref('ElPradoChinCreekBev/hole' + hole + '/ping').on('value', function (snapshot) {
